...
class DBInterface(object):
    ...
    @requires_lock
    def do_edit_product_location(self, location, serialnum):
        '''
        Changes the location of x manufacturer's/serialnum in products.
        '''
        result = self._sqlconn.execute(''.join([
            'UPDATE products ',
            'SET location=? ',
            'WHERE products.serialnum = ?'
        ]), (location, serialnum))
        self._sqlconn.commit()
        print "Serial Number '{}'' updated to {}".format(serialnum, location)

    @requires_lock
    def do_list_products(self):
        '''
        Returns a list of all products currently in the inventory ordered by manufacturer.
        '''
        result = self._sqlconn.execute(''.join([
            'SELECT * FROM products ',
    ...
    def do_search_products(self, manufacturer):
        '''
        Returns a list of all products with the manufacturer given.
        '''
        result = self._sqlconn.execute(''.join([
            'SELECT * FROM products ',
            'WHERE manufacturer = ? ',
    ...
    def do_add_new_product(self, product_dictionary):
        ...
        '''
        result = self._sqlconn.execute(''.join([
            'INSERT INTO products (manufacturer, model, serialnum, location) ',
            'VALUES (?, ?, ?, ?)'
        ]), (
            product_dictionary['manufacturer'],
            product_dictionary['model'],
            product_dictionary['serialnum'],
            product_dictionary['location']
        ))
        self._sqlconn.commit()
